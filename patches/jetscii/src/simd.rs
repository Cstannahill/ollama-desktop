    ...
    T: PackedCompareControl,
{
    ...
    unsafe fn cmpestrm(&self, haystack: &[u8], leading_junk: usize) -> Option<usize> {
        ...
        let mask = if T::CONTROL_BYTE == _SIDD_CMP_EQUAL_ORDERED {
            _mm_cmpestrm::<{ _SIDD_CMP_EQUAL_ORDERED }>(
                self.0.needle(),
                self.0.needle_len(),
                haystack,
    ...
    unsafe fn cmpestri(&self, haystack: *const u8, haystack_len: i32) -> Option<usize> {
        ...
        let location = if T::CONTROL_BYTE == _SIDD_CMP_EQUAL_ORDERED {
            _mm_cmpestri::<{ _SIDD_CMP_EQUAL_ORDERED }>(
                self.0.needle(),
                self.0.needle_len(),
                haystack,
                haystack_len,
